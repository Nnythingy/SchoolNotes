...
int BinarySearchTree<T>::rank(T item) {
	int value = 0;
	_rank(_root, _search(_root, item), &value);
	if (exist(item)) return value;
	return -1; 
}
template <class T>
void BinarySearchTree<T>::_rank(TreeNode<T>* test, TreeNode<T>* looking, int* value) {
	if (!test) return;
	_rank(test->_left, looking, value);
	if (test->_item < looking->_item) *value += 1;
	_rank(test->_right, looking, value);
}
