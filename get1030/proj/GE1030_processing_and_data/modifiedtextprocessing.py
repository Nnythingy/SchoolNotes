...
def compare_raters(directory,includes_index=True):
    ...
    for number,fileitem in enumerate(files,1):
        ...
        if sorted(temp_df.columns)!= ["chorus","label"]:
            print(f"ERROR in {fileitem}: The columns must be named 'label' and 'chorus', in small caps.")
            return None

        #values should only be pos, neg and neu
        #if sorted(temp_df['label'].unique()) not in sorted(["Happiness","Sadness","Anger","Fear","Disgust","Surprise"]):
        #    print(sorted(temp_df['label'].unique()))
        #    print(f"ERROR in {fileitem}: The 'label' column can only contain the values 'Happiness','Sadness','Anger','Fear','Disgust','Surprise' in smal caps.")
        #    return None
        ...
        if number == 1:
            ...
            temp_df = temp_df.drop('chorus', axis=1)
...
class Analyzer:
    ...
    def __init__(self,file_location):
        ...
        if "chorus" not in df.columns:
            print("ERROR: One of the columns must be named 'chorus', in small caps.")
            return None
        ...
        #if sorted(df['ground truth'].unique()) != ["Happiness","Sadness","Anger","Fear","Disgust","Surprise"]:
        #    print("ERROR: The 'label' column can only contain the values 'Happiness','Sadness','Anger','Fear','Disgust','Surprise' in smal caps." +
        #        "\nIf 'no agreement' was found you need to resolve this through discussion or by bringing in a new labeler.")
        #    return None

        errors = 0
        for k,v in df.iterrows():
            prob = analyzer.predict(v["chorus"]).probas
    ...
    def examine_labels(self,actual="Happiness",predicted="Happiness"):
