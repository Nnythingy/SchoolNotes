...
def generate_file_process_graph(auditbeat_file, allowed_prefix=None):
    ...
    with open(auditbeat_file, 'r') as f:
        for line in f:
            ...
            if executable_path not in process_list:
                process_list[executable_path] = {
                    'name': name,
            ...
            if file_path not in process_list[executable_path]['files']:
                process_list[executable_path]['files'][file_path] = 0
            process_list[executable_path]['files'][file_path] += 1
    ...
    with open('file_process.json', 'w') as f:
...
def draw_graph(file_process_graph, output_filename):
    ...
    for pid, process in file_process_graph['processes'].items():
        ...
        graph.node(str(process['pid']), label=label, shape='box', style='rounded,filled', fillcolor='lightblue')
        for file_path, count in process['files'].items():
            graph.edge(str(process['pid']), file_path, label=str(count))
...
if __name__ == '__main__':
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print('Usage: python3 file_process_graph.py <auditbeat_log.ndjson> <output_filename> [allowed_file_prefix]')
        print('Example: python3 file_process_graph.py auditbeat.ndjson file_process_graph.png /home')
        sys.exit(1)
