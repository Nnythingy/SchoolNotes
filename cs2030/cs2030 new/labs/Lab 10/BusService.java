...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    CompletableFuture<Scanner> sc = BusAPI.getBusStopsServedBy(serviceId)
            .thenApply(x -> new Scanner(x));

    CompletableFuture<Set<BusStop>> stops = sc.thenApply(x -> x.useDelimiter("\n"))
                  .thenApply(x -> x.tokens())
                  .thenApply(x -> x.map(line -> line.split(",")))
                  .thenApply(x -> x.map(fields -> new BusStop(fields[0], fields[1])))
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(bs -> {
      return bs.stream()
