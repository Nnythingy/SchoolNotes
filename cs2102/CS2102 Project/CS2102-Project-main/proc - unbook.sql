...
BEGIN
    ...
    IF EXISTS (SELECT * FROM Sessions s
        WHERE in_id = s.booker_id AND s.room = in_room AND s.floor = in_floor AND s.date = in_date
        AND s.time = in_start AND s.time = in_end)
    THEN 
        DELETE From Sessions s1 WHERE 
        WHERE s1.room = in_room AND s1.floor = in_floor AND s1.date = in_date
        AND s1.time = in_start AND s1.time = in_end)
        -- this one I think will automatically remove entries from Joins?
    ELSE
        RAISE NOTICE "Booking cannot be removed";
    END IF;
END;
$$ LANGUAGE plpgsql;
...
/* --Booker books empty room for 1 hour
CALL book_room(5, 5, CURRENT_DATE + 1, '08:00:00', '09:00:00', 2);
SELECT * FROM Sessions; -- 2 rows
SELECT * FROM Joins; -- 2 row */


/* --Booker deletes empty booking - failure
CALL approve_meeting(1, 1, CURRENT_DATE - 1, '09:00:00', '10:00:00', 2);
SELECT * FROM Sessions; -- 2 rows
*/

/* --Random deletes existing booking - failure
CALL approve_meeting(5, 5, CURRENT_DATE + 1, '08:00:00', '09:00:00', 5);
SELECT * FROM Sessions; -- 2 rows
*/

/* --Booker deletes his own booking - success
CALL approve_meeting(5, 5, CURRENT_DATE + 1, '08:00:00', '09:00:00', 2);
SELECT * FROM Sessions; -- 2 rows
*/